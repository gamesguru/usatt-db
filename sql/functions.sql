...
OR REPLACE FUNCTION players.hsgames() RETURNS TABLE(
  ...
  created_at TIMESTAMP, tournament_id INT
) AS $$
SELECT
  sg.game_id,
...
FROM
  ...
  INNER JOIN players.users AS u2 ON sg.player2_id = u2.user_id $$ LANGUAGE SQL;
--
--
--
-- #2
...
FROM
  ...
  INNER JOIN players.users AS u4 ON dg.player4_id = u4.user_id $$ LANGUAGE SQL;
