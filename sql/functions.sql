...
OR REPLACE FUNCTION players.hsgames() RETURNS TABLE(
  ...
  created_at TIMESTAMP, tournament_tag VARCHAR
) AS $$
SELECT
  sg.game_id,
...
FROM
  ...
  LEFT JOIN players.tournaments AS t ON sg.tournament_id = t.tournament_id $$ LANGUAGE SQL;
--
--
--
-- #2
...
FROM
  ...
  LEFT JOIN players.tournaments AS t ON dg.tournament_id = t.tournament_id $$ LANGUAGE SQL;
